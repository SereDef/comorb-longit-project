...
standardized <- (grepl('std', out_folder)) # standardize parameters
...
make_df <- function(name1, times1, name2, times2, 
  ...
  # print(colSums(is.na(as.data.frame(cbind(d1,d2)))))
  ...
  # data[,c('sex','ethnicity','m_edu','p_edu')]
  # print(summary(df))
  ...
  # Stratify
  if (!is.null(strata)) { 
    df <- df[!is.na(data[strata[1]]) & data[strata[1]]==strata[2],]
  ...
  samp = df[rowSums(is.na(df[,c(paste0(var1,1:ncol(d1)), paste0(var2,1:ncol(d2)))])) != ncol(d1)*2, ]
  # samp = df[rowSums(is.na(df)) != ncol(df), ]
  
  c = round(cor(samp, use='pairwise.complete.obs', method='pearson'),2)
  ...
  # if (return_time_scale=='years'){
  #   if (!all(med_ages1==times1) | !all(med_ages2==times2)) { cat('\nCheck time input!\n') }
  # }
...
riclpm_formula <- function(var1='dep', var2='cmr', 
                           n_ocs=NULL, 
                           meas_time=list(c(), c()), 
                           rel='*', # specify equality using power or linear relations 
                           stationarity=TRUE,
                           standardized=TRUE,
                           verbose=TRUE) {
  ...
  if (stationarity & standardized) { l = 'NA*' } else { l = '1*' }
  ...
  if (stationarity & standardized) { w1 = '1*' } else { w1 = '' }
...
run_lavaan <- function(param, group=NULL, normalize=TRUE) {
  ...
  dep_summ <- do.call(cbind, lapply(data[,sel(param[[1]], param[[2]])], summary))
  cmr_summ <- do.call(cbind, lapply(data[,sel(param[[3]], param[[4]])], summary))
  ...
  if (!lavaan::lavInspect(m, 'converged')){
    ...
    # Fit measures
    fit_meas  <- as.data.frame(lavaan::fitmeasures(m))
    # Parameters
    stad_esti <- lavaan::standardizedSolution(m, type="std.all")[,c('est.std','ci.lower','ci.upper','pvalue')] # same but standardized
    estimates <- lavaan::parameterEstimates(m)

    save(fit_meas, estimates, stad_esti, c, times, dat_summ,
...
foreach(i=1:length(models)) %dopar% {
  ...
  mt = run_lavaan(param=params)
  
  # Stratify by sex
  mm = run_lavaan(param=params, group= c('sex','1'))
  mf = run_lavaan(param=params, group= c('sex','2'))
